...
use ggez::{GameError, Context, GameResult};

use crate::{WINDOWS_WIDTH, WINDOWS_HEIGHT};
...
fn render_ui_system(manager: &mut EntityManager, ctx: &mut Context) -> GameResult<()> {
    ...
    for entity in buttons {
        let transform = Transform::new(
            Vec2::new(WINDOWS_WIDTH / 2f32, 10f32),
            Vec2::zero()
        );
...
impl Game {
    fn setup(&mut self) {
        ...
        space_game_btn.add_component(Button { title: "Space Game".to_string() });
    }
}
